...
export function buildWhatsAppMessage(params: {
  ...
  const { item, publicUrl, template } = params;
  const start = formatDateLine(parseMaybeDate(item.startDateISO || item.startDate));
  const end = formatDateLine(parseMaybeDate(item.endDateISO || item.endDate));
  ...
  if (item.type === 'special') {
    return [
      `Hi! I would like to redeem or reserve the special: ${values.title}.`,
      `Valid until: ${values.end_date}`,
      `Here is the Sayso link: ${values.public_url}`,
    ].join('\n');
  }

  const firstLine = start
    ? `Hi! I would like to book for ${values.title} on ${values.start_date}.`
    : `Hi! I would like to book for ${values.title}.`;
...
export function buildWhatsAppDeepLink(params: { number: string; message: string }) {
  return `https://wa.me/${params.number}?text=${encodeURIComponent(params.message)}`;
}
